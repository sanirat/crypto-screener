...
def structure_pass(high: np.ndarray, low: np.ndarray, close: np.ndarray, struct_n: int = 60) -> bool:
    # Higher highs + higher lows using swing pivots on last struct_n points.
    ...
    # Fallback structure proxy (MA stacking)
    s20 = sma(c, 20)
    s60 = sma(c, 60)
...
def is_probable_stablecoin(row) -> bool:
    ...
    if any(x in sym for x in ["usdt", "usdc", "dai", "tusd", "fdusd", "busd", "usdd", "usde"]):
        return True
    # avoid too many false positives; keep this conservative
    return False
...
with st.sidebar:
    st.header("Universe")
    max_pages = st.number_input("Market-cap pages to scan (250 coins/page)", min_value=1, max_value=40, value=8)
    min_mcap = st.number_input("Min market cap (USD)", min_value=0, value=50_000_000, step=10_000_000)
    min_vol = st.number_input("Min 24h volume (USD)", min_value=0, value=5_000_000, step=1_000_000)
...
st.caption("Data source: CoinGecko public API. Scanning lots of coins may hit rate limits. Start with fewer pages and increase gradually.")
...
for i, row in dfm.iterrows():
    ...
    try:
        ...
        # Structure from OHLC if available
        try:
            ohlc = cg_ohlc(coin_id, days=90)
            if len(ohlc) >= struct_n:
